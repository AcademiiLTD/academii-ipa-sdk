...
using WebSocketClient = NativeWebSocket.WebSocket;
using WebSocketStateAlias = NativeWebSocket.WebSocketState;
...
namespace Academii.WebSocket.Client
{
 ...
 public class AcademiiWebSocketAPIClient : IDisposable
 {
  ...
  private readonly Dictionary<string, WebSocketClient> _connections = new Dictionary<string, WebSocketClient>();
  ...
  public async Task ConnectToEndpointAsync(string endpointKey, string fullUrl, CancellationToken cancellationToken = default)
  {
   ...
   var webSocket = new WebSocketClient(fullUrl, $"auth.{_authToken}");
  ...
  {
   if (!_connections.TryGetValue(endpointKey, out var webSocket) || webSocket.State != WebSocketStateAlias.Open)
  ...
  /// </summary>
  public WebSocketStateAlias GetConnectionState(string endpointKey)
  {
   return _connections.TryGetValue(endpointKey, out var webSocket) ? webSocket.State : WebSocketStateAlias.Closed;
  }
  ...
  {
   return _connections.TryGetValue(endpointKey, out var webSocket) && webSocket.State == WebSocketStateAlias.Open;
  }
